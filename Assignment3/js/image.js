...
function imageList(node, imgarray) {
    var i = 0;
    var content = "";
    for(i = 0; i < imgarray.length; i++) {
        content += "<figure><img src='" + imgarray[i].url + "' alt='" 
        + imgarray[i].alt + "'><figcaption>" + imgarray[i].caption + "</figcaption></figure>"
    }
    node.innerHTML = content;
}

function outPut() {
    var imgContainer = document.getElementById("imageList");
    imageList(imgContainer, images);
}
